...
21. [分布式](#分布式)
    ...
    - [分布式任务池](#分布式任务池)
22. [向量打分](#向量打分)
23. [内嵌 JavaScript](#内嵌-javascript)
24. [运行和构建](#运行和构建)
25. [测试](#测试)
26. [开发期契约](#开发期契约)
27. [代码风格](#代码风格)
...
把 blas 异步请求集中到另一个线程,blas 只是换了个核调用。多了一次线程切换 /
交接的开销,更可能把这个核占满,总吞吐可能还会降低。
...
> **绝不要为任何生产需求依赖本库。** 契约默认 OFF，因此它们检查的一切
...
若 app 用了 `(igropyr blas)` 且有原生 BLAS，按核铺开 node 并把 BLAS 每进程设为单线程——否则它的内部线程会和 node 争抢、过度订阅核。在进程启动前设好环境变量（BLAS 在加载共享库时读取）：Linux/FreeBSD 用 `OPENBLAS_NUM_THREADS=1` / `OMP_NUM_THREADS=1`，macOS 用 `VECLIB_MAXIMUM_THREADS=1`。详见[向量打分](#向量打分)。
