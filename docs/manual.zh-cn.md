...
扫描是一次 FFI 调用,不可被抢占。它在返回前一直占着 scheduler 的 OS 线程,
该线程上所有绿进程都在等。绿进程共享同一个 OS 线程,所以把扫描换个 actor
做没有意义。停顿只能在各自拥有 OS 线程的单位间分摊:独立 OS 进程(按核铺开的
`SO_REUSEPORT` fleet)或独立 node,各扫各的副本。有 N 个这样的单位时,每线程
占空比是 `q·s/N`(查询速率 × 扫描时长 ÷ OS 线程数)。把搜索汇聚到少数专用进程
正好相反:多一跳、多一个队列,没有好处。

扫描是 CPU 计算而非 I/O,所以吞吐由物理 CPU 定上限:总核数除以每次扫描的
CPU 秒。在已饱和的机器上把扫描 offload 到另一线程,只是在忙碌的核之间搬运
工作、不增加算力,改变的是延迟落点而非吞吐。更多 node 只有在带来更多核或机器
时才提吞吐。语料涨到扫描不再亚毫秒时,把扫描分块、块间让出调度点,或分片
