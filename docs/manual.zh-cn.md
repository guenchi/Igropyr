...
`(igropyr metrics)` 是**信号侧**：它采集、并用同一份数字序列化成三种格式，
不关心读端是 Prometheus、浏览器还是 Scheme 程序。一个 middleware 记录每个
请求，各 endpoint 渲染：
...
- `(make-metrics)` → collector；`(metrics-middleware collector)` 记录每个
  请求的状态码与时延。
- `(metrics-endpoint collector srv)` 渲染 Prometheus 文本（每状态码计数、
  时延 summary、连接/pool/uptime gauge）。
- `(metrics-json collector srv)` / `(metrics-sexpr collector srv)` 把整份
  快照（含集群视图）以 JSON 或一个 s-表达式返回；`(metrics-snapshot
  collector srv)` 把同一份数据作为 Scheme 值给进程内调用方。JSON 与 sexpr
  共用一个快照构造器，永不漂移。

**业务计数器**搭同一个 collector——无需注册，直接计数：
...
输入在调用点校验（cast 即发即忘，坏名字或非数字增量必须在这里就响亮失败，
而不是崩掉共享 collector）；标签排序，使一个标签集的两种顺序仍是一条序列；
`igropyr_` 前缀保留给内建族。

**浏览器监控页**是独立库 `(igropyr dashboard)`——表现层与信号解耦。它自带
一个自包含页面（内联 CSS/JS、无外部资源、离线可用：requests/s 与延迟
sparkline、连接/pool 仪表、每状态码计数、所有计数族，每 2 秒刷新），并提供
一个**默认绑回环**的开箱 admin 监听器：
...
(admin-listen metrics srv `((port . 9090) (auth . ,(token-guard verify))))
```
...
- `(admin-listen collector srv [opts])` → server 专用监听器，**默认回环**。
  `(host . ...)`、`(port . 9090)`、`(auth . middleware)` 先应用、
  `(prefix . "/")`、`(html . X)`；`http-shutdown!` 停止。

数据路由暴露运维细节，`admin-listen` 因此默认回环；若挂到公开 app，请像
`/metrics` 一样保护路由（`(auth . …)`、反向代理或网络策略）。

**集群视图**：节点上（`node-start!` 之后）宣告一次本机摘要，所有同样宣告过
的 peer 就出现在快照的 `cluster` 成员里（uptime、连接数、requests、5xx、
pool）——经已有 node 链路 `rcall` 聚合，peer 无需暴露 HTTP：

```scheme
(metrics-announce! metrics srv)
```

每个 peer 调用限时 1 秒；没宣告/超时/畸形回复的 peer 渲染成 `no data`（字段
为空），不拖垮端点。没跑 `node-start!` 时 `cluster` 为 null、监控页集群表隐藏。
...
FFI 调用不可被抢占,所以一次全扫会把它的 scheduler 所在的**那一个 OS
线程**冻住一个扫描时长——该 scheduler 上**所有绿进程**一起等,不只是调用者。
要让它不拖累别人,就得把扫描摊开,但**单位要说准**:绿进程(actor)全都
**共享同一个 OS 线程**,所以把扫描换个绿进程做**毫无用处**,线程照样被冻。
能分摊阻塞的,只有各自**拥有独立 OS 线程**的单位——独立 OS 进程(按核铺开的
`SO_REUSEPORT` fleet)或独立 node。让每个这样的单位就地扫自己的副本,每线程
停顿占空比 `q·s/N`(查询速率 × 扫描时长 ÷ OS 线程数)就可忽略。**不要**把搜索
汇聚到少数专用进程:那给每次搜索加一跳,还平白多一个饱和队列。

一句吞吐上的诚实校准:扫描是 **CPU-bound**(它在真算,不是等 I/O),所以它的
吞吐上限 = 总物理 CPU ÷ 每次扫描的 CPU 秒——由硬件定死,和你怎么在核上摆
线程无关。在**同一台已饱和**的机器上把扫描 offload 到另一线程**不创造 CPU**,
因此不提吞吐;它只是重排**扫描何时发生**(尾延迟),还可能因交接略降。分布到
更多 node **只有在带来更多物理核/机器时**才提吞吐。若单个语料涨到扫描不再
亚毫秒,先把扫描分块、块间让出调度点,或分片 scatter-gather 到各 node——杠杆
是**更多 CPU、以更多独立 node 的形式加入**,不是重排线程。

### BLAS 线程数

原生 BLAS(尤其 OpenBLAS)可能为一次 `sgemv` 开**它自己的**线程。若你又一核
一 node,这些内部线程就会**过度订阅**核——node 和 BLAS 线程抢同一批 CPU。这种
布局下要把 BLAS 每进程钉成一线程。库里**没有**这个开关(即便有也只能是残缺
的——Accelerate 没有运行时 setter);线程数是 **BLAS 在加载其共享库时读取的
环境变量**,所以必须在**进程启动前**设好——**启动后再设无效**。这个时序正是
关键:

```sh
# 在拉起进程的地方(rc.d/systemd 单元、部署脚本)设,exec scheme 之前
OPENBLAS_NUM_THREADS=1    # OpenBLAS(Linux/FreeBSD)
OMP_NUM_THREADS=1         # 编了 OpenMP 的 OpenBLAS,以及其他 OpenMP BLAS
VECLIB_MAXIMUM_THREADS=1  # macOS Accelerate / vecLib
```

按你的后端设对应那一组(`OPENBLAS_*` 和 `OMP_*` 一起给是保险)。只在一核一
node 布局下钉成 1;单 node 且有空闲核时可以让 BLAS 保持多线程,但小语料下线程
开销通常盖过收益。
