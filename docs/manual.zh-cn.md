...
扫描是一次阻塞的 FFI 调用:它在返回前一直占着 scheduler 的 OS 线程,该线程上
所有绿进程都在等。把这份阻塞分布到各 igropyr node 上分摊,可以提高集群的
总响应能力。绿进程共享同一个 OS 线程,所以把扫描换个 actor 做没有意义;停顿
只能在各自拥有 OS 线程的单位间分摊,也就是独立 OS 进程(按核铺开的
`SO_REUSEPORT` fleet)或独立 node,各扫各的副本。有 N 个这样的单位时,每线程
占空比是 `q·s/N`(查询速率 × 扫描时长 ÷ OS 线程数)。

扫描是 CPU 计算而非 I/O,所以吞吐由物理 CPU 定上限。在每个 CPU 核已经跑一个
node 时,把扫描 offload 到单独线程并不能减少总响应时间——核已饱和,工作只是
在核之间挪动——反而增加线程切换开销。更多 node 只有在带来更多核或机器时才
提吞吐。语料涨到扫描不再亚毫秒时,把扫描分块、块间让出调度点,或分片
