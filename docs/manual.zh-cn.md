...
node 时,把扫描 offload 到单独线程并不能减少总响应时间:扫描只是换了个核跑,
而核已饱和,于是它在那儿改为争抢。这多了一次线程切换和交接,还更可能把那个核
占满,所以总吞吐甚至可能下降。更多 node 只有在带来更多核或机器时才提吞吐。语料涨到扫描不再亚毫秒时,把扫描分块、块间让出调度点,或分片
