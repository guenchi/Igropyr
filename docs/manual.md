...
not have, so the lower layers do not port.
...
If the server crashes, the caller immediately gets `#(gen-server-error server-died reason)` (because the caller monitors the server). The last slot is small enough to print. A reason that already is — a symbol or string of at most 200 characters, a fixnum, a boolean, the empty list, a character, the eof object, the void value — arrives unchanged and stays `eq?`-comparable; anything else is replaced by a **label**, one naming its shape where the kind is a known one (`"#(do-thing ...)"` for a request vector, `"#<number>"` for a bignum, `"#<its-type>"` for a record) and `"#<value>"` where it is not. The same holds for `timeout` and `calling-self`, whose slot describes the caller's own message. Nothing prints your value to build the label, which is the point: reading the error cannot walk into whatever the message referred to, and cannot run a record writer that was going to be expensive, raise, or keep the port. A symbol longer than 200 characters, and any gensym, arrive as labels rather than themselves, so `eq?` on the slot holds up to that length and not beyond.
...
- `(session-clear! session)` → void — clear the session's data **and rotate the session id**, sending a `Set-Cookie` for the new one. This is what makes it a logout rather than an emptying: an id captured beforehand no longer names anything a later login will write into. On a session that is still new — its id has not reached the client yet — it clears without rotating, so a logout never emits two cookies. Like `session-regenerate!`, it must be called **before the response is sent**, since the replacement cookie has to travel with it
