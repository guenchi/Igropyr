...
If the server crashes, the caller immediately gets `#(gen-server-error server-died reason)` (because the caller monitors the server). The last slot is small enough to print. A reason that already is — `'normal`, a symbol or string of at most 200 characters, a number that is a fixnum — arrives unchanged and stays `eq?`-comparable; anything else is replaced by a **label** naming its shape, such as `"#(do-thing ...)"` for a request vector or `"#<number>"` for a bignum. The same holds for `timeout` and `calling-self`, whose slot describes the caller's own message. Nothing prints your value to build the label, which is the point: reading the error cannot walk into whatever the message referred to, and cannot run a record writer that was going to be expensive, raise, or keep the port. A symbol longer than 200 characters, and any gensym, arrive as labels rather than themselves, so `eq?` on the slot holds up to that length and not beyond.
