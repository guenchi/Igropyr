...
(let ((target (string-append root "/a.bin"))
  ...
  (if macos?
      (check "F_FULLFSYNC is issued for the file and for the directory"
             ...
             (ops))
      (display "  note  fullfsync arm not judged: F_FULLFSYNC is macOS-only\n"))
  ...
  (check "the bytes read back"
         (equal? payload
                 (call-with-port (open-file-input-port target)
                                 get-bytevector-all)))
  (let ((second (string->utf8 "second, shorter")))
...
(let ((dir (string-append root "/sub")))
  ...
  (reset!)
  (durable-dir-ensure! dir)
  ;; zero CREATION calls -- the parent fsync legitimately repeats, and
  ;; mtime could not tell this apart from a re-create with equal bytes
  ...
  (let ((r (guard (e (#t e))
    ...
    (let ((raised (filter (lambda (e) (eq? (caddr e) 'raised)) (trace))))
      (check "the failed attempt left a 'raised entry"
             ...
             (trace)))
    ;; ...and NOTHING after it: no fsync, no rename on a write that
    ;; never happened
    (check "fail closed: no call follows the failed step"
           (and (= 0 (count-op 'fsync)) (= 0 (count-op 'rename)))
           (ops))))
...
(let ((r (guard (e (#t (if (durable-error? e) 'durable 'refused)))
