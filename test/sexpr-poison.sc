...
(define (run-child! root name seconds)
  (let ((f (string-append dir "/child.sc")))
    (call-with-output-file f
      (lambda (o) (display (string-append "(import (chezscheme) (igropyr sexpr)) (write (guard (e (#t 'refused)) (sexpr->string (list 'a (string->symbol \"" name "\"))))) (newline)") o)) 'truncate)
...
(define (witness-built?)
  (eqv? 0 (exit-code (system (string-append "grep -q '(define (wire-symbol? s)' " witness "/sexpr.sc && sed -i.bak 's/(define (wire-symbol? s)/(define (wire-symbol? s) (string->number s)/' " witness "/sexpr.sc && grep -q '(define (wire-symbol? s) (string->number s)' " witness "/sexpr.sc")))))
...
(let ((r (exit-code (run-child! witness poison 5))))
  (check "witness: with the conversion first the poisoned name hangs the child (timeout exit 124)" (eqv? r 124) r (child-output)))

(let* ((t0 (now-ms)) (r (exit-code (run-child! tree poison 5))) (ms (- (now-ms) t0)))
  (check "poisoned symbol name: sexpr->string refuses, no hang" (and (eqv? r 0) (equal? (child-output) "refused\n")) r (child-output))
...
(check "a name the raw conversion accepts but the reader would not is refused (+i)" (guard (e (#t #t)) (sexpr->string (list (string->symbol "+i"))) #f))
(check "a short radix form is refused, not converted" (guard (e (#t #t)) (sexpr->string (list (string->symbol "#e1024"))) #f))
(check "a name with an exponent but no # is refused" (guard (e (#t #t)) (sexpr->string (list (string->symbol "1e99999999"))) #f))
