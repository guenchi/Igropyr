        ...
        (let ((mrefs (arm-monitors! 3)))
          ...
          (let ((pk (park! 'cleanup-before-round 1 100)) (link ($node-link-pid 'b)))
            ...
            (let ((w (inject-barrier-wait (car pk) 'cleanup-before-round 8000)))
              (check "N7a: the primary parked with the record published" (and (pair? w) (eq? (cdr w) link) (eqv? (stat 'cleanup-records) 1)) w)
              (expect-down! "N7a: node-down b" 'b 6000)
              (when (pair? w) (kill (cdr w) 'n7a-primary-killed) (guard (e (#t (void))) (inject-release! (car pk))))
              (cleanup! pk 'cleanup-before-round)
              ;; the original reaper is alive but its periodic scan is 60 s away: the record stays pending
              (sleep-ms 1500)
              (check "N7a: premise -- the record is still pending (no periodic scan in reach)" (eqv? (stat 'cleanup-records) 1) (stat 'cleanup-records))
              ;; kill the reaper; hold its warden replacement before it registers; the record must still be pending
              (let ((pkid (park! 'warden-child-before-register 1 100)) (r0 (reaper)))
                (kill r0 'n7a-reaper-killed)
                (let ((k (inject-barrier-wait (car pkid) 'warden-child-before-register 5000)))
                  (check "N7a: the replacement reaper parked before registering" (pair? k) k)
                  (check "N7a: premise -- still pending during the outage" (eqv? (stat 'cleanup-records) 1) (stat 'cleanup-records))
                  (let ((t0 (now-ms)))
                    (when (pair? k) (send (cdr k) (vector 'inject-resume (car pkid))))
                ...
                (cleanup! pkid 'warden-child-before-register))))))
      ...
      (let ((b1 (open-peer "b" BOOT-X 1)))
        ...
        (let ((mrefs (arm-monitors! 70)))
          (let ((pk (park! 'cleanup-before-round 1 100)) (link ($node-link-pid 'b)))
            (plain-peer-close! b1)
            (let ((w (inject-barrier-wait (car pk) 'cleanup-before-round 8000)))
              (check "N7b: the primary parked with 70 obligations published" (and (pair? w) (eq? (cdr w) link)) w)
              (expect-down! "N7b: node-down b" 'b 6000)
              (when (pair? w) (kill (cdr w) 'n7b-primary-killed) (guard (e (#t (void))) (inject-release! (car pk))))
              (cleanup! pk 'cleanup-before-round)
