...
;; ARGON2ID needs OpenSSL 3.2+; a 3.0/3.1 libcrypto has every other KDF
;; here. Only the checks that actually DERIVE are gated -- the malformed
;; input and cost-bomb checks below use argon2id-shaped strings but are
;; rejected by the parameter guards before any KDF runs, so they must keep
;; running everywhere, and would be silently lost to a blanket skip.
(define argon2? (kdf-argon2id-available?))
(unless argon2?
  (display "  SKIP argon2id derivations: this libcrypto has no ARGON2ID.\n")
  (display "       It needs OpenSSL 3.2+ -- Debian 12 and Ubuntu 22.04 ship\n")
  (display "       3.0. Everything else in this suite still runs; to cover\n")
  (display "       argon2id, run on a host with OpenSSL >= 3.2.\n"))
...
;; where it is absent, the failure must be the documented one -- not a
;; segfault, and not a hash silently derived by some other primitive
(unless argon2?
  (check "argon2id-absent-raises"
    (guard (e (#t #t)) (kdf-argon2id (u "password") (u "0123456789abcdef") 2 64 1 32) #f)))
