...
(import (chezscheme) (igropyr tls) (only (igropyr tls-core) tls-error-attribution tls-session-new! tls-session-retire! tls-live-session-count))
...
(let ((ctx (tls-listen-context! (in-dir "good.pem") (in-dir "good.key"))))
  (check "C1: a matching certificate/key pair yields a context" (and (integer? ctx) (not (zero? ctx))) ctx))
