...
;; the same shape INSIDE the length limit: green here is the shape check's alone
;; (the raw conversion returns 10^99 for it, not #f)
(t "digits: the exponent form is refused within max-len too" (digits->exact "#e1e99" 10) #f)
...
(t "signed: max-len bounds the digits, the sign is not counted" (signed-digits->exact "-123456789012345678901" 21) -123456789012345678901)
(t "signed: max-len + 1 digits refused" (signed-digits->exact "-1234567890123456789012" 21) #f)
...
(t "signed: the exponent form is refused within max-len too" (signed-digits->exact "#e1e99" 20) #f)
(t "signed: a signed exponent form within max-len is refused" (signed-digits->exact "-#e1e99" 20) #f)
;; ---- hex-digits->exact ----------------------------------------------------------
...
(t "fraction: the exponent form refused within max-len too" (decimal-fraction->number "#e1e99" 8) #f)
