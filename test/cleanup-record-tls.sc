    ...
    (let ((base (list (tls-live-session-count) (tls-live-watcher-count)))
      ...
      (unless (pair? s)
        ...
        (let ((caller (spawn (lambda () (send main (vector 'call-outcome (guard (e (#t e)) (rcall 'b 'svc '(q) 20000))))
                                        (let fwd () (receive (m (send main (vector 'stray m)) (fwd)))))))
              (m (monitor-remote 'b 'svc)))
          (let ((f1 (read-frame s 3000)) (f2 (read-frame s 3000)))
            (check "N13: the peer received one call frame and one mon frame" (and (pair? f1) (pair? f2) (memq (car f1) '(call mon)) (memq (car f2) '(call mon)) (not (eq? (car f1) (car f2)))) (list f1 f2)))
          (let ((link ($node-link-pid 'b)))
            ...
            (receive (after 800 (check "N13: no duplicates" #t)) (`#(remote-down b svc ,r2) (check "N13: no duplicates" #f r2)) (`#(stray ,m) (check "N13: no duplicates" #f (list 'stray m))))
            (kill caller 'cell-done)
