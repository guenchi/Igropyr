#!chezscheme
;;; Helper: a second OS process acting as node <name> that LISTENS on <port>
;;; and never dials. Derived from node-child.sc (which dials 'a); the
;;; registrar cells need a peer that node 'a authorises a dial TO, so the
;;; direction is the whole point.
;;; Usage: scheme --script node-child.sc <name> <port> <secret>
;;; Listens on 127.0.0.1:<port> (node 'a dials it), then:
