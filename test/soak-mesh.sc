    ...
    (let ((hp (getenv "SOAK_HTTP_PORT")))
      (when hp
        (http-listen (string->number hp)
          ...
          ;; SOAK_BACKLOG raises the listen backlog for load generators that
          ;; open their whole concurrency in one burst; default is the
          ;; library's, and the kernel's somaxconn still clamps it
          (list (cons 'host host) (cons 'workers 4)
                (cons 'backlog (string->number (or (getenv "SOAK_BACKLOG") "8192")))))
