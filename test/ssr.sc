...
;; A bellard libquickjs also counts as "present": loaded on its own it
;; does not resolve JS_FreeValue, so the suite then fails at engine boot
...
(define (run-cache-checks r p)
  ...
  ;; bytes API: a hit returns the UTF-8 bytes of the same HTML, no re-render
  (check (L p "render-bytes")
    (equal? (ssr-render/bytes r "render" '(("t" . "A")) '((key . "/a")))
            (string->utf8 "<h1>A</h1>")))
  (check (L p "render-bytes-no-rerender") (= 1 (N)))
  (let-values (((ok bv) (ssr-try-render/bytes r "render" '(("t" . "A")) '((key . "/a")))))
    (check (L p "try-render-bytes")
      (and ok (bytevector? bv) (string=? (utf8->string bv) "<h1>A</h1>"))))
