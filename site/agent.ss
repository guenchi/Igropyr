   ...
   `(header
      (div (@ (class "wrap"))
        ...
        (p (@ (class "tag")) "Two AI coding agents — one to " (b "build") " new "
           ...
           "Node/Express app.")
        (p (@ (class "sub")) "Self-contained Markdown · API map labeled for Igropyr 1.3.0")
        (div (@ (class "cta"))
   ...
   `(section (@ (id "dev"))
      (div (@ (class "wrap"))
        ...
        (p (@ (class "lead")) "Hand it a spec, a single endpoint, or the framework "
           ...
           "direct blocking style, let-it-crash error handling, and a v1.3.0 "
          ...
          (tbody
           ...
           ,(row '((code "res.json(obj)"))
                 '((code "(send-json! res alist)")
                   " — string keys, and an array is a vector"))
...
(write-file "agent.html"
  (render-page
   ...
   (string-append "Two AI coding agents for Igropyr: igropyr-dev writes idiomatic "
                  ...
                  "re-architects Node.js/Express services into it. The module map in "
                  "igropyr-dev is labeled for Igropyr 1.3.0.")
   body))
