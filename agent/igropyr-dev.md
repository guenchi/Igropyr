...
| auth | auth (middleware) req-claims token-guard session-guard. **Three channels, TWO shapes**: `auth` takes a *token verifier* `(lambda (token) claims-or-#f)`; `app-ws` and `app-rpc` take a *request guard* `(lambda (req) claims-or-#f)`. Both are one-argument procedures, so passing a guard to `auth` is accepted at boot and misbehaves per request; `token-guard` lifts a verifier into a guard, and nothing lowers a guard into a verifier. HTTP via `(app-use app (auth verifier))`; app-ws takes a guard as 4th arg (refused BEFORE the 101 handshake, plain 401); app-rpc takes a guard as 4th arg (refusal answers the sexpr datum `(error unauthorized)`; a two-argument handler `(lambda (args req))` reads req-claims for per-tag authorization). auth options: `(optional . #t)` lets tokenless requests through (invalid tokens still 401), `(on-fail . proc)` overrides the refusal body; token-guard reads Bearer, falls back to ?token= on ws (`(query . #f)` disables) |
