...
- `(igropyr auth)`: auth req-claims token-guard session-guard — three channels (HTTP middleware, ws upgrades, sexpr RPC) but TWO shapes: `auth` takes a token verifier `(lambda (token) claims-or-#f)`, `app-ws`/`app-rpc` take a request guard `(lambda (req) claims-or-#f)`. Both are one-argument procedures, so the wrong one is accepted at boot; `token-guard` lifts verifier→guard, nothing lowers guard→verifier
