...
- `(igropyr auth)`: auth req-claims token-guard session-guard — three channels (HTTP middleware, ws upgrades, sexpr RPC) and three non-interchangeable shapes: token verifier `(lambda (token) claims-or-#f)` for `auth`/`token-guard`; request guard `(lambda (req) claims-or-#f)` for `app-ws`/`app-rpc`; middleware `(lambda (req res next))` for `app-use` and `admin-listen`'s `auth` option, which is what `auth` returns. `auth` lifts verifier→middleware, `token-guard` lifts verifier→guard, nothing converts downward; all are procedures, so the wrong one installs silently and fails on the first request
