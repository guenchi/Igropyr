...
- **非阻塞 HTTP 和 WebSocket 客户端** — 出站 `http-get` / `http-post` 和 `ws-connect`，
  都支持 async DNS（libuv 线程池）并使用同样的“暂停调用者”模型
- **异步文件读取** — 静态文件在 libuv 线程池上读取，因此大型或冷文件读取不会阻塞 scheduler
...
- **加固** — 严格 `Content-Length` 校验、每请求响应隔离、响应头注入防护、
  static mount 边界检查、带重组上限的 WebSocket frame 校验
...
`#(http-client-error msg)`。不直接支持 https；TLS-only endpoint 应通过 proxy 访问
（见下面 HTTPS）。
...
默认每个请求都经过 worker pool，这带来 crash retry 和 stuck-kill，
代价是每个请求约 4 次进程间消息和一次上下文切换。对于纯粹且能快速返回的 handler，
这次往返是主要的 per-request 成本。把这类 route 标记为 *fast*，
它会在连接的 reader 进程中 inline 执行，跳过 pool：

```scheme
(app-get-fast app "/" (lambda (req res) (send-html! res "hi")))
;; 还有 app-post-fast / app-put-fast / app-delete-fast
```

在 trivial keep-alive route 上测量，这会把 per-request CPU 降低约 20%
（单个饱和 core 上大约 145k -> 200k req/s）。取舍是：
fast handler 失去 pool 的容错能力 — crash 会被捕获并对该连接返回 `500`（不重试），
阻塞或循环的 handler 只会冻结自己的连接（没有 30 s stuck-kill）。
只把它用于纯粹、快速的 handler；有副作用、数据库调用或重工作的内容应保留在默认 pooled path。

## 容错语义

这些语义适用于 pooled route（默认）；无需配置：
...
跨进程共享请通过 Redis，并用 Redis pub/sub 作为跨进程 event bus。

## HTTPS / TLS

Igropyr 讲明文 HTTP；TLS 应在前面的 reverse proxy 终止。这是标准部署方式，
...
libuv-callback 不变量记录在 [DEVELOPING.zh-CN.md](DEVELOPING.zh-CN.md)。
