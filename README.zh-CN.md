...
- **分布式 actor** — 把节点连成 mesh（`(igropyr node)`）：`rsend`/`rcall`
  ...
  静态、gossip 或 Redis——gossip 是去中心成员管理，骑 node 链路本身：只需配
  几个种子地址，其余成员的地址随记录自行到达）
...
peer 在已鉴权的 node 链路上 push-pull。成员地址随记录传播，所以配置一个种子
联系人就足以认识（并被认识）所有人——种子节点自己可以零种子运行。记录携带
incarnation（属主的启动时间戳；重启碾压旧世代的一切记录）和只由属主推进的心跳；
停止推进的记录在每个节点自己的时钟上于 `ttl-ms` 内老化（最坏约 2 倍），陈旧回声
无法复活已移除的成员。哪里都不存在"从零发现"：和所有成员系统一样，第一个
联系人——这里是种子，下面 redis 策略里是 Redis 的地址——来自配置而非魔法；
gossip 只是把它缩小到几个 peer 地址，并去掉了那个外部服务。

**redis** 策略是同一套过期语义、但由共享存储而非 P2P 仲裁：每个节点心跳进一个
按过期时间戳打分的有序集合，崩溃的节点自己掉出。已经在跑 Redis 时用它。
