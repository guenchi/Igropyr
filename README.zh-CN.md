...
- **WebSocket** — 在同一端口上执行 RFC 6455 upgrade；每个 socket 都是自己的绿色进程，
  因此 server push 就是一次消息发送。升级可在握手前被守卫——`app-ws` 接受一个可选的
  鉴权守卫，未鉴权的对端拿不到 socket
...
- **鉴权** — `(igropyr auth)` 是格式中立的*鉴权角色*，横跨两个信道：`auth`
  中间件用 `Bearer` token 守卫 HTTP 路由（接受任意校验器——今天是
  `(jwt-verifier key)`——否则 401），`token-guard` / `session-guard` 在握手前
  守卫 WebSocket 升级
- **JWT** — `(igropyr jwt)` 签发和验证 HS256 token（算法钉死、恒定时间
  比较、严格 base64url、fail-closed），是插入 `(igropyr auth)` 的凭证格式
...
- **S3 兼容对象存储** — `(igropyr sigv4)` 做 AWS Signature V4 签名（AWS 官方
  测试向量钉死正确性），`(igropyr s3)` 在 HTTP client 之上讲 S3 REST API：
  put / get / 服务端 copy / delete / 分页 list — AWS S3、Cloudflare R2、MinIO 通用
- **向量打分（embedding 检索内核）** — `(igropyr blas)`：一次调用把查询向量对
  row-major float32 矩阵按内存带宽打满分（FFI 走 Accelerate / OpenBLAS），
  无 BLAS 的主机自动回退纯 Scheme 循环——慢但正确，随处可跑
- **内嵌 JavaScript** — `(igropyr quickjs)` 经 QuickJS 在进程内跑固定 JS
  bundle（内存/栈/时间受限、crash-only 重建）；用户输入是数据，永不是代码
- **运维就绪** — rate limiting、全局错误处理器，监控信号支持 Prometheus / JSON /
  sexpr 与自定义业务计数器（`metrics-count!`），并自带一个带集群视图、默认绑回环
  admin 端口的自包含浏览器监控页
...
- **分布式 actor** — 把节点连成 mesh（`(igropyr node)`）：`rsend`/`rcall`
  ...
  分布式任务池（`(igropyr dpool)`）、以及自动发现（`(igropyr cluster)`，
  静态、Redis 或 gossip——去中心成员管理，骑 node 链路本身：只需配几个
  种子地址，其余成员的地址随 gossip 记录自行到达）
...
- **开发期契约** — `(igropyr checked)` 提供 `define-checked` /
  `define-checked-record` 检查内部不变量，由 `IGROPYR_CONTRACTS` 在编译期
  门控：`off`（默认）编译为空、零残留，`full` 注入检查并对违规的
  过程 / 参数报 blame。内建库在 debug 构建下为其导出带边界契约
...
有原生 BLAS 时走 `cblas_sgemv`（macOS Accelerate、Linux/FreeBSD
OpenBLAS），没有时走纯 Scheme 循环——`blas-available?` 告诉你在哪条道上，
正确性从不依赖原生库。top-k、阈值、存储仍归应用；这里只管以内存带宽
完成扫描。
...
调度提示：FFI 调用不可被抢占，一次全量扫描会让调用方 scheduler 停顿
一个扫描时长（5k×512 f32 约 0.2ms，100k 约 5ms）。把停顿**摊薄**——每个
进程就地扫自己的副本，每进程停顿占空比 `q·s/N` 可忽略；不要把搜索汇聚到
少数专用进程。占空比若真涨上来，先把扫描分块、块间让出调度点，或分片
scatter-gather——始终分散，永不集中。

## 内嵌 JavaScript

`(igropyr quickjs)` 在进程内嵌一个 JavaScript 引擎（QuickJS），用来跑一段
**固定的**、构建期烘焙进去的 JS bundle——需要逐字节对齐的参考实现、沙盒化
表达式求值、JS 模板等。用户输入是字符串**参数**，永远不是代码：
...
C shim（`quickjs-shim.c`）里做了加固：内存上限、栈上限、wall-clock 中断
截止，以及 **crash-only 重建**——抛异常或失控的调用会丢弃整个 JS 堆、从
bundle 重新引导（`qjs-generation` 记重建次数），一次坏调用毒不到下一次。
引擎由 mutex 串行化、每次调用在关中断下跑，因此一次调用会阻塞其 OS 线程
一个调用时长（通常亚毫秒，最坏 `timeout-ms`）——延迟敏感路径请在调用方限制
输入大小。原生 shim 是构建产物（`build-quickjs-shim.sh`，需装 QuickJS）；
没有它库照常导入，`qjs-boot!` 会报 shim 缺失。
...
Metrics：一个 middleware 把每个请求记进同一个 collector，`(igropyr metrics)`
再用同一份数字渲染三种格式——信号与格式无关，不关心读端是 Prometheus、
浏览器还是 Scheme 程序：
...
(app-get app "/metrics"     (metrics-endpoint m srv))  ; Prometheus 文本
(app-get app "/stats.json"  (metrics-json m srv))      ; JSON 快照
(app-get app "/stats.sexpr" (metrics-sexpr m srv))     ; sexpr 快照
;;   igropyr_requests_total{status="200"} 1234
;;   igropyr_request_duration_ms_sum 45210  ...  igropyr_pool_busy 3
```

`metrics-snapshot` 把同一份数据以 Scheme 值返回给进程内调用方。业务计数器
也搭这个 collector——无需注册，直接 `(metrics-count! m "jobs_done_total" '() 5)`。

表现层是独立库 `(igropyr dashboard)`，页面与信号从不耦合。它自带一个
自包含的浏览器监控页（内联 CSS/JS、无外部资源、离线可用：requests/s 与
延迟 sparkline、连接/pool 仪表、每状态码计数、所有计数族，每 2 秒刷新），
并且——因为监控面暴露运维细节——提供一个**默认绑回环**的开箱 admin 监听器：

```scheme
(import (igropyr dashboard))
;; 挂到你已有的 app（自行加保护）：
(mount-dashboard! app m srv)                 ; GET /dash , /dash/data[.sexpr]
;; 或专用 admin 端口，默认 127.0.0.1，带鉴权：
(admin-listen m srv `((port . 9090) (auth . ,(token-guard verify))))
```

前端可替换：传 `(html . <string|procedure>)` serve 你自己的页面——用
`send-file!` serve 一个放在 web 根之外的文件，或用
[Goeteia](https://github.com/guenchi/Goeteia) app 读 sexpr 端点——它和数据
路由解耦，源文件可放任意位置、随你渲染。

节点上（`node-start!` 之后）宣告一次本机摘要，所有同样宣告过的 peer 就会
出现在快照的 `cluster` 成员里（uptime、连接数、requests、5xx、pool）——数据
经由已有的 node 链路 `rcall` 聚合，peer 无需额外暴露 HTTP：

```scheme
(metrics-announce! m srv)
```

数据路由暴露运维细节，`admin-listen` 因此默认绑回环；若改为挂到公开 app，
请像 `/metrics` 一样保护路由（`(auth . …)`、反向代理或网络策略）。任何监听器
的绑定网卡现在都可配——`(app-listen app port '((host . "127.0.0.1")))` 即可
只留本机。
...
(cluster-start `((name . "myapp")                     ; 完全无共享存储
...
**gossip** 策略是同一套过期语义的去中心化版本：每个节点维护一张复制的成员表，
每周期与几个随机 peer 在已鉴权的 node 链路上 push-pull。成员地址随记录传播，
所以配置一个种子联系人就足以认识（并被认识）所有人——种子节点自己可以零种子
运行。记录携带 incarnation（属主的启动时间戳；重启碾压旧世代的一切记录）和只由
属主推进的心跳；停止推进的记录在每个节点自己的时钟上于 `ttl-ms` 内老化（最坏
约 2 倍），陈旧回声无法复活已移除的成员。哪里都不存在"从零发现"：和所有成员
系统一样，第一个联系人——redis 策略里是 Redis 的地址，这里是种子——来自配置
而非魔法；gossip 只是把它缩小到几个 peer 地址,并去掉了那个外部服务。
...
metrics.sc     监控信号：Prometheus / JSON / sexpr，集群快照
dashboard.sc   监控页 + 开箱 admin 监听器（默认绑回环）
client.sc  非阻塞出站 HTTP client（async DNS）
sigv4.sc   AWS Signature V4 请求签名（纯函数）
s3.sc      S3 兼容对象存储（AWS S3 / R2 / MinIO）
blas.sc    向量打分内核：可选 CBLAS sgemv，纯 Scheme 回退
quickjs.sc 进程内嵌 JS 引擎（QuickJS，经 C shim）
